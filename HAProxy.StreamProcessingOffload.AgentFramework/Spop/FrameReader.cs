...
public static class FrameReader
{
    ...
    public static bool TryReadFrame(ref ReadOnlySequence<byte> readableBuffer, SpopFrameMetadata frameMetadata, out ReadOnlySequence<byte> framePayload)
    {
        ...
        var result = reader.TryReadFrame(frameMetadata, out framePayload);
    ...
    public static bool TryReadFrame(ref this SequenceReader<byte> input, SpopFrameMetadata frameMetadata, out ReadOnlySequence<byte> framePayload)
    {
        ...
        if (!input.TryReadInteger(out var length))
        {
            return false;
        }
        ...
        // <FRAME-TYPE:1 byte>
        if (!frameReader.TryReadByte(out var type))
        {
            return false;
        }

        // <FLAGS:4 bytes>
        if (!frameReader.TryReadInteger(out var flags))
        {
            return false;
        }

        // <STREAM-ID:varint>
        if (!frameReader.TryReadVariableInteger(out var streamId))
        {
            return false;
        }

        // <FRAME-ID:varint>
        if (!frameReader.TryReadVariableInteger(out var frameId))
        {
            return false;
        }
        ...
        frameMetadata.Flags = (Frame)flags;
    ...
    public static bool TryReadInteger(ref this SequenceReader<byte> reader, out int value) => reader.TryReadBigEndian(out value);

    public static bool TryReadByte(ref this SequenceReader<byte> reader, out byte value) => reader.TryRead(out value);

    public static bool TryReadVariableInteger(ref this SequenceReader<byte> reader, out long value)
    {
        if (reader.TryRead(out var nextValue))
        {
            value = nextValue;
            if (value < 240)
            {
    ...
    {
        value = default;
        if (!TryReadVariableInteger(ref reader, out var length))
        {
            return false;
        }
    ...
    public static void DecodeListOfMessagesPayload(in ReadOnlySequence<byte> sequence, out IEnumerable<SpopMessage> value)
    {
        ...
        while (!reader.End)
        {
            if (!reader.TryReadString(out var messageName))
            {
                return;
            }
            ...
            if (!TryReadByte(ref reader, out var nbArgs))
            {
                return;
            }
            ...
            for (byte i = 0; i < nbArgs; i++)
            {
                ...
                if (!reader.TryReadTypeData(out var keyvalue))
                {
                    return;
                }
    ...
    public static void DecodeListOfActionsPayload(in ReadOnlySequence<byte> sequence, out IEnumerable<SpopAction> value)
    {
        ...
        while (!reader.End)
        {
            ...
            if (!reader.TryReadByte(out var actionType))
            {
                return;
            }

            if (!reader.TryReadByte(out var nbArgs))
            {
                return;
            }

            if (!reader.TryReadByte(out var scope))
            {
                return;
            }

            if (!reader.TryReadString(out var actionName))
            {
                return;
            }
            ...
            if (nbArgs == 3)
            {
                if (!reader.TryReadTypeData(out actionValue))
                {
                    return;
                }
            }
    ...
    {
        value = default;
        if (!reader.TryRead(out var dataTypeByte))
        {
            return false;
        }
        ...
        else
        {
            switch ((DataType)dataType)
            {
                ...
                { return true; }
                case DataType.Int32:
                    if (reader.TryReadInt32(out var int32))
                    { value = int32; return true; }
                    break;
                case DataType.Int64:
                    if (reader.TryReadInt64(out var int64))
                    { value = int64; return true; }
                    break;
                case DataType.Uint32:
                    if (reader.TryReadUint32(out var uint32))
                    { value = uint32; return true; }
                    break;
                case DataType.Uint64:
                    if (reader.TryReadUint64(out var uint64))
                    { value = uint64; return true; }
                    break;
                case DataType.Ipv4:
                    if (reader.TryReadIpv4(out var address))
                    { value = address; return true; }
                    break;
                case DataType.Ipv6:
                    if (reader.TryReadIpv6(out var address6))
                    { value = address6; return true; }
                    break;
                case DataType.String:
                    if (reader.TryReadString(out var @string))
                    { value = @string; return true; }
                    break;
                case DataType.Binary:
                    if (reader.TryReadBinary(out var binary))
                    { value = binary; return true; }
                    break;
                case DataType.Boolean:
                    break;
                case DataType.ReservedBegin:
                    break;
                case DataType.ReservedEnd:
                    break;
                default:
                    throw new ApplicationException(string.Format(CultureInfo.InvariantCulture, "Unable to parse data type: {0}", dataType));
            }
    ...
    public static bool TryReadIpv4(ref this SequenceReader<byte> reader, out IPAddress value)
    {
        ...
        if (!reader.TryCopyTo(stackBuffer))
        {
            return false;
        }
    ...
    public static bool TryReadInt64(ref this SequenceReader<byte> reader, out long value) => reader.TryReadVariableInteger(out value);

    public static bool TryReadInt32(ref this SequenceReader<byte> reader, out int value)
    {
        if (reader.TryReadVariableInteger(out var longValue))
        {
            value = (int)longValue;
            return true;
        }
